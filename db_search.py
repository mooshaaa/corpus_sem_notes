...
def get_sentences_with_lemma_only(db_file, lemma, doc_name=None, gender=None):
    ...
    if (doc_name != None) and (gender != None):
        cursor.execute("""
            ...
            WHERE WORD.lemma = ? AND DOCS.doc_name = ? AND DOCS.gender = ?
        """, (lemma, doc_name, gender))
        ...
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.lemma = ? AND DOCS.doc_name = ?
        """, (lemma, doc_name))
    else:
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
...
def get_sentences_with_wordform_only(db_file, wordform, doc_name=None, gender=None):
    ...
    if (doc_name != None) and (gender != None):
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.word = ? AND DOCS.doc_name = ? AND DOCS.gender = ?
        """, (wordform, doc_name, gender))
    elif (doc_name == None) and (gender != None):
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
            FROM WORD
        ...
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.word = ? AND DOCS.doc_name = ?
        """, (wordform, doc_name))
    else:
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
...
def get_sentences_with_lemma_tag(db_file, lemma, tag, doc_name=None, gender=None):
    ...
    if (doc_name != None) and (gender != None):
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.lemma = ? and WORD.tag = ? AND DOCS.doc_name = ? AND DOCS.gender = ?
        """, (lemma, tag, doc_name, gender))
    elif (doc_name == None) and (gender != None):
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
            FROM WORD
        ...
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.lemma = ? and WORD.tag = ? AND DOCS.doc_name = ?
        """, (lemma, tag, doc_name))
    else:
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
...
def get_sentences_with_wordform_tag(db_file, wordform, tag, doc_name=None, gender=None):
    ...
    if (doc_name != None) and (gender != None):
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.word = ? and WORD.tag = ? AND DOCS.doc_name = ? AND DOCS.gender = ?
        """, (wordform, tag, doc_name, gender))
    elif (doc_name == None) and (gender != None):
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
            FROM WORD
        ...
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.word = ? and WORD.tag = ? AND DOCS.doc_name = ?
        """, (wordform, tag, doc_name))
    else:
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
            FROM WORD
...
def get_sentences_with_tag(db_file, tag, doc_name=None, gender=None):
    ...
    if (doc_name != None) and (gender != None):
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.tag = ? AND DOCS.doc_name = ? AND DOCS.gender = ?
        """, (tag, doc_name, gender))
    elif (doc_name == None) and (gender != None):
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
        ...
        cursor.execute("""
            ...
            JOIN SENT ON SENT.sent_id = WORD.sent_id
            JOIN DOCS on DOCS.doc_id = SENT.doc
            WHERE WORD.tag = ? AND DOCS.doc_name = ?
        """, (tag, doc_name))
    else:
        cursor.execute("""
            SELECT DISTINCT WORD.sent_id, WORD.word_position, DOCS.doc_id
...
# %time
search('A рамка', 'sem_notes_corp.db', doc_name='Компоненты значения.')